...
pub fn get_repositories_directory() -> PathBuf {
    ...
    if !repositories_directory.exists() {
        std::fs::create_dir_all(&repositories_directory).expect("Unable to create repositories directory");
    }
...
pub fn get_repositories_configs() -> Vec<Result<std::fs::DirEntry, std::io::Error>> {
    let external_versions_directories = std::fs::read_dir(get_repositories_directory()).expect("Could not read external repositories directory").filter(|entry| entry.as_ref().unwrap().path().is_dir());
...
pub fn sync_version_repositories() {
    ...
    if let Some(version_repositories) = config.version_repositories {
        ...
        for version_repository in &version_repositories {
            ...
            let version_repository_name = version_repository.split("/").last().unwrap().replace(".git", "");
            ...
            if !version_repository_path.exists() {
                ...
                command.arg("clone").arg(version_repository).arg(&version_repository_name);
                command.current_dir(&external_versions_directory);
                command.output().expect("Unable to clone version repository");
            } else {
                ...
                clean_command.output().expect("Unable to clean version repository");

                pull_command.arg("pull");
                pull_command.current_dir(&version_repository_path);
                pull_command.output().expect("Unable to pull version repository");

                reset_command.arg("reset").arg("--hard");
                reset_command.current_dir(&version_repository_path);
                reset_command.output().expect("Unable to clean version repository");
            }
        }
